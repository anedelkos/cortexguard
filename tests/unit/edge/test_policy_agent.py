...
async def test_cooldown_suppresses_duplicate_key_within_window(
    ...
    mock_deps["blackboard"].get_latest_state_estimate.return_value = _make_state(ts)

    first = _make_anomaly("id-1", "sensor_spike", ts)
    await agent._handle_anomaly_event(first)

    assert mock_deps["blackboard"].set_remediation_policy.call_count == 1

    # Second event immediately after — real elapsed time is milliseconds, well within 30s cooldown
    second = _make_anomaly("id-2", "sensor_spike", ts)
...
async def test_cooldown_expires_and_allows_refire(
    ...
    ts = datetime.now(UTC)
    agent = PolicyAgent(**mock_deps, remediation_cooldown_s=0.05)
    mock_deps["blackboard"].get_latest_state_estimate.return_value = _make_state(ts)
    ...
    # Wait for the 0.05s cooldown to expire
    await asyncio.sleep(0.1)

    ts2 = datetime.now(UTC)
    mock_deps["blackboard"].get_latest_state_estimate.return_value = _make_state(ts2)
    second = _make_anomaly("id-2", "sensor_spike", ts2)
    await agent._handle_anomaly_event(second)
